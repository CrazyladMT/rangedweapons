core.register_globalstep(function(dtime, player)
	for _, player in pairs(core.get_connected_players()) do
		...
		if controls.LMB then
			if itemstack:get_definition().RW_gun_capabilities then
				if itemstack:get_definition().RW_gun_capabilities.automatic_gun and itemstack:get_definition().RW_gun_capabilities.automatic_gun > 0 then
					rangedweapons_shoot_gun(itemstack, player)
					player:set_wielded_item(itemstack)
				end
			end

			if itemstack:get_definition().RW_powergun_capabilities then
				if itemstack:get_definition().RW_powergun_capabilities.automatic_gun and itemstack:get_definition().RW_powergun_capabilities.automatic_gun > 0 then
		...
		if u_meta:get_float("rw_cooldown") <= 0 then
			if itemstack:get_definition().loaded_gun ~= nil then
				if itemstack:get_definition().loaded_sound ~= nil then
					core.sound_play(itemstack:get_definition().loaded_sound, {pos = player:get_pos()}, true)
				end

				itemstack:set_name(itemstack:get_definition().loaded_gun)
				player:set_wielded_item(itemstack)
			end

			if itemstack:get_definition().rw_next_reload ~= nil then
				...
				itemstack:set_name(itemstack:get_definition().rw_next_reload)
				player:set_wielded_item(itemstack)
			end
		end
	end
